<?php
    ...
    if(isset($_GET["delete_invite"])){
        $invitecode = $_GET["delete_invite"];
        $sql = "DELETE FROM `invites` WHERE `inviteCode`='$invitecode'";
        $db = mysqli_connect('localhost', 'root', 'atomicasb123', 'file-host');
        if($result = mysqli_query($db, $sql)){
            $_SESSION["banmsg"] = "<div class='card' <div class='card-body'><h3 class='card-text' style='color: green;'>Deleted Invite Code!</h3></div><br>";
            header("location: invite-management");
        }
    }
    ...
    <div class="collapse navbar-collapse" id="navbarCollapse">
      <ul class="navbar-nav mr-auto">
        ...
        <li class="nav-item active">
          <a class="nav-link" target="_self" href="#">Invite Management <span class="sr-only">(current)</span></a>
        </li>
    ...
    <div class="wrap-login100">
      ...
      <span class="login100-form-title p-b-26">
        ...
        if($role == "Owner" || $role == "Admin"){
            echo "<h5 class='card-title'><br>Invite Management</h5>";
              echo "<div class='banmsg'>" . $_SESSION["banmsg"] . "</div>";
            $sql = "SELECT * FROM `invites` ORDER BY id DESC";
            if($result = mysqli_query($db, $sql)){
                if(mysqli_num_rows($result) > 0){
                    while($row = mysqli_fetch_array($result)){
                      $invite_author = $row["inviteAuthor"];
                      $invite_code = $row["inviteCode"];
                      $bannedurl = "
                      <hr class='rounded'>
                      <div class='flex'>
                      <p class='flex-child-small'>Invite Author: <br><a style='color: white;'>$invite_author</a></p>
                      <p class='flex-child-small'>Invite Code: <br><a style='color: white;'>$invite_code</a></p>
                      <p class='flex-child-small'><br><a href='?delete_invite=" . $invite_code . "' style='color: white;'>Delete</a></p>
                      </div>";
                        echo $bannedurl;
                    }
                }
