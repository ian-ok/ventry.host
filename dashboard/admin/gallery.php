...
if (isset($_GET['page']) && $_GET['page']!="") {
	...
	$result_count = mysqli_query($con,"SELECT COUNT(*) As total_records FROM `uploads`");
	$total_records = mysqli_fetch_array($result_count);
	$total_records = $total_records['total_records'];
    $total_no_of_pages = ceil($total_records / $total_records_per_page);
	$second_last = $total_no_of_pages - 1; // total page minus 1

    $result = mysqli_query($con,"SELECT * FROM `uploads` ORDER BY `id` DESC LIMIT $offset, $total_records_per_page");
    ?>
		<div class="row-new">
		<?php
					if($uploads == "0"){
...
<p style="color: grey;"class='card-text'>Uploader:<br><a style='color: white;'><?php echo $row['username'] ?></a></p>
