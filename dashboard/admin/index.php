<?php
    ...
    if (!isset($_SESSION['username'])) {
      ...
      if(isset($_GET["unban_user"])){
        ...
        $uuid12 = $_GET["uuid"];
        $username12 = $_GET["username"];
        
        $script = "UPDATE `users` SET `banned`='false' WHERE `secret`='$user_secret'";
        $execute = mysqli_query($db, $script);
        chmod("../../uploads/$uuid12/$username12", 0777);
        header("Location: https://mhills.de/dashboard/admin/");
        exit;
    }
    if(isset($_GET["ban_user"])){
        ...
        $script = "UPDATE `users` SET `banned`='true' WHERE `secret`='$user_secret'";
        $execute = mysqli_query($db, $script);
        chmod("../../uploads/$uuid12/$username12", 0000);
        header("Location: https://mhills.de/dashboard/admin/");
        exit;
    }
      ...
      $invitecount = mysqli_num_rows($results251);
        ...
        //Invite Count
        $query21 = "SELECT * FROM `users` ORDER BY `id` ASC";
    ...
    if(isset($_GET["delete_user"])){
        $user_secret = $_GET["delete_user"];
        $sql12 = "SELECT * FROM `users` WHERE `secret`='$user_secret'";
        $result12 = mysqli_query($db, $sql12);
        $row54 = mysqli_fetch_assoc($result12);
        $username12 = $row54["username"];

        $sql1 = "DELETE FROM `users` WHERE `secret`='$user_secret'";
        mysqli_query($db, $sql1);
        $sql2 = "DELETE FROM `uploads` WHERE `username`='$username12'";
    ...
    -ms-box-shadow: 0 5px 10px 0px rgba(0, 0, 0, 0.1);" src="<?php echo $row["discord_avatar"] ?>" alt="Face 1">
                    ...
                    <div class="row">
                        <div class="col-12">
                            <div class="card">
                                <div class="card-header">
                                    <h4 class="card-title">Users (<?php echo $usercount?>)</h4>
                                </div>
                                <div class="card-body px-3 py-4-5">
                                    ...
                                    <table class="table table-striped dataTable-table" id="table1">
                                        <thead>
                                            <tr>
                                                ...
                                                <th style="width: 26.0933%;"><a style='text-decoration: none;' class="dataTable-sorter">User</a></th>
                                                <th style="width: 14.3109%;"><a style='text-decoration: none;' class="dataTable-sorter">Email</a></th>
                                                <th style="width: 20.3109%;"><a style='text-decoration: none;' class="dataTable-sorter">Registered at</a></th>
                                                <th style="width: 14.3109%;"><a style='text-decoration: none;' class="dataTable-sorter">Secret</a></th>
                                                <th style="width: 6.3109%;"><a style='text-decoration: none;' class="dataTable-sorter">Uploads</a></th>
                                                <th style="width: 18.3109%;"><a style='text-decoration: none;' class="dataTable-sorter">Role</a></th>
                                                <th style="width: 18.3109%;"><a style='text-decoration: none;' class="dataTable-sorter">Ban</a></th>
                            ...
                            for ($i = 0; $i < $usercount; ++$i) { 
                                ...
                                echo "<tr>
                                <td>" . $row21["id"] . "</td>
                                <td><a style='text-decoration: none;' href='https://mhills.de/profile/" . $row21["id"] . "'>" . $row21["username"] . "</a> <span class='badge bg-primary'>".$row21["role"]."</span>                                ";

                                if($row21["discord_nitro"] == "Nitro Subscriber"){
                                    echo "<img id='nitro-badge' style='height: 20px' src='https://discordapp.com/assets/e04ce699dcd2fd50d352a384511687a9.svg'>";
                                }
                                if($row21["role"] == "Server Booster"){
                                    echo "<img id='booster-badge' style='height: 20px; margin-left: 2px' src='https://raw.githubusercontent.com/Mattlau04/Discord-SVG-badges/master/SVG/boost3month.svg'>";
                                }
                                if($row21["role"] == "Owner"){
                                    echo "<img id='owner-badge' style='height: 20px; margin-left: 4px' src='https://raw.githubusercontent.com/Mattlau04/Discord-SVG-badges/4466e3e494c9d13cc9267728a74e0d31825ebc32/SVG/Server_owner.svg'>";
                                }

                                echo "</td>
                                <td>" . $row21["email"] . "</td>
                                <td>" . $row21["reg_date"] . "</td>
                                <td><a id='blurtext' style='color: white;'>" . $row21["secret"] . "</a></td>
                                <td>" . $row21["uploads"] . "</td>
                                <td><span class='badge bg-primary'><a style='text-decoration: none;' href='change-role?user=" . $row21["secret"] . "'>Change Role</a></span></td>
                                ";
                                    if($row21["banned"] == "false"){
                                        echo "<td><span class='badge bg-danger'><a style='text-decoration: none;' href='?ban_user&secret=" . $row21["secret"] . "&uuid=" . $row21["uuid"] . "&username=" . $row21["username"] . "'>Ban</a></span></td>";
                                    }
                                    else{
                                        echo "<td><span class='badge bg-success'><a style='text-decoration: none;' href='?unban_user&secret=" . $row21["secret"] . "&uuid=" . $row21["uuid"] . "&username=" . $row21["username"] . "'>Unban</a></span></td>";
                                    }
                                echo "
                                <td><span class='badge bg-warning'><a style='text-decoration: none;' href='?delete_user=" . $row21["secret"] . "'>Delete</a></span></td>
